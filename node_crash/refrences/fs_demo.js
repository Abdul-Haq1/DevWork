...
fs.appendFile(path.join(__dirname, '/test', 'documentation.txt'),'/n Because when run cmd node init this folder dowanloaded automaticaly.', (err) => {
    if(err) throw err;
    console.log("file is updated...");
});  
...
fs.rename(
    ...
    path.join(__dirname, '/test', 'mainDocmentation.txt'),
    err => {
        if(err) throw err;
